...
{
	// create output file names
	QString fntFileName = fileName + ".cpp";
	...
	foreach (auto font, fontLst) {
		...
		fontStream
		  ...
		  << "//   face        : " << font.m_font << "\n"
		  << "//   size        : " << font.m_size << "\n"
		  << "//   bold        : " << (font.m_style & FontRec::BOLD   ? "true" : "false") << "\n"
		  << "//   italic      : " << (font.m_style & FontRec::ITALIC ? "true" : "false") << "\n"
		  << "//   smooth      : " << (font.m_style & FontRec::SMOOTH ? "true" : "false") << "\n"
		  << "//   padding     : " << packer.borderTop << "," << packer.borderRight << "," << packer.borderBottom << "," << packer.borderLeft << "\n"
		  << "//   line height : " << fontMetrics.height() << "\n"
		  << "//   base        : " << fontMetrics.ascent() << "\n"
		  << "//   scaleW      : " << texture.width() << "\n"
		  << "//   scaleH      : " << texture.height() << "\n"
		  << "//   alphaChnl   : " << (transparent ? 0 : 4) << "\n"
		...
		// output array header
		fontStream << "font_t font_" << count << "[" << font.m_glyphLst.size() << "] = {" << endl;
		...
		foreach (auto glyph, font.m_glyphLst) {
			...
			if (glyph == font.m_glyphLst.last()) fontStream << "}  "; else fontStream << "}, ";
	...
	// Save fonts array
	fontStream << "font_t *fonts[" << fontLst.count() << "] = {" << endl;
	for (int i = 0; i < fontLst.count(); i++) {
		fontStream << "\tfont_" << i;
		if (i+1 < fontLst.count()) fontStream <<  ",\n"; else fontStream << "\n";
	}
	fontStream << "};\n\n";
	...
	fontStream << "uint8_t fonts_texture[" << data.size() << "] = {";
